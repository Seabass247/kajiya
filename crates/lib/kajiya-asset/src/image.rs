...
use crate::mesh::GpuImageImmediate;

#[derive(Clone, Hash, PartialEq, Eq)]
pub enum ImageSource {
    File(PathBuf),
    Memory(Bytes),
    LoadImmediate(PathBuf),
}
...
impl LoadImage {
    ...
    pub fn new(source: &ImageSource) -> anyhow::Result<Self> {
        match source {
            ...
            ImageSource::LoadImmediate(path) => {
                let image_bytes = LoadFile::new(path)?.load_immediate()?;
                Ok(Self::Immediate(image_bytes))
            },
        }
    }

    pub fn create(self) -> anyhow::Result<RawRgba8Image> {
        let bytes: Bytes = match self {
            // Note: `Bytes` does internal reference counting, so this clone is cheap
            LoadImage::Lazy(bytes) => return Err(anyhow::anyhow!("Missing attribute")),
...
pub struct CreateGpuImageImmediate {
    pub image: RawRgba8Image,
    pub params: super::mesh::TexParams,
}

impl CreateGpuImageImmediate {
    pub fn create(&self) -> anyhow::Result<GpuImageImmediate> {
        let src = &self.image;
